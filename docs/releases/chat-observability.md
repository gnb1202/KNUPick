...
2026-09-13. A 대기 화면과 B 실행 기록을 구현했다. 피드백·검토 화면은 다음 커밋에서 이어서 구현한다. 운영 DB·운영 배포는 변경하지 않았다. 일반 검색·추천·검색 정책과 모델은 유지한다.
...
추가 마이그레이션은 `20260913091908_chat_observations.sql`이다. 새 테이블은 RLS를 켜고 anon/authenticated의 직접 접근을 제거한다. 서비스 역할만 쓰기 RPC를 실행한다. 기존 공지 데이터는 수정하지 않는다.
...
- `npm test -- --maxWorkers=2`: 104개 통과. PGlite의 실제 SQL에서 RLS·함수 권한·늦은 쓰기·보관 만료·사용자 삭제를 확인했다.
