...
> 테스트 대화 관측은 [구현·검증·운영 전환 안내](docs/releases/chat-observability.md)를 참고하세요. 대기 표시, 동의한 테스트 계정의 실행 기록, 답변 평가, `/admin/chat-observations`의 본인 기록 검토와 표본 통계를 구현했습니다. 기본 수집은 OFF이며 이 변경의 운영 DB 적용·배포는 아직 수행하지 않았습니다.
