...
> 테스트 대화 관측은 [구현·검증·운영 전환 안내](docs/releases/chat-observability.md)를 참고하세요. 대기 표시, 동의한 테스트 계정의 실행 기록, 답변 평가, `/admin/chat-observations`의 본인 기록 검토와 표본 통계를 제공합니다. 코드 기본값은 OFF이며 운영에서는 서버 허용 목록과 계정별 동의로 수집을 제한합니다. 실제 활성화 상태는 운영 환경과 배포 기록을 기준으로 확인하세요.

> 최신 챗봇 구현·검증·한계는 [전공 탐색 및 원문 근거 변경 기록](docs/releases/chat-discovery-v1.md)을 확인하세요. 전공별 주제 탐색, 조건 안에서의 벡터 검색, 행사 날짜를 포함한 종료 처리, 최초 답변의 원문 근거, SSE 오류 표시를 포함합니다. [후속 질문·카드 참조](docs/releases/legacy-followup-v1.md)도 유지합니다. 아래의 기존 프로젝트 소개는 기준 커밋 당시의 기록이며, 최신 동작과 배포 상태는 변경 기록과 PR을 기준으로 합니다.
