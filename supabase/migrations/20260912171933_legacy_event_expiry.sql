-- Expiry fallback: application deadline, event end, event start. Unknown dates stay eligible.
BEGIN;
...
RETURNS TABLE(post jsonb,similarity float) LANGUAGE sql STABLE SECURITY INVOKER SET search_path=public,extensions,pg_temp AS $$
  WITH candidates AS MATERIALIZED (
    SELECT p.* FROM public.posts p WHERE p.embedding IS NOT NULL AND (include_expired OR coalesce(p.deadline,p.event_end_date,p.event_start_date) IS NULL OR coalesce(p.deadline,p.event_end_date,p.event_start_date) >= as_of)
  )
  SELECT to_jsonb(p)-'embedding',(1-(p.embedding <=> query_embedding::text::vector(1024)))::float
  FROM candidates p WHERE (1-(p.embedding <=> query_embedding::text::vector(1024)))>match_threshold
  ORDER BY p.embedding <=> query_embedding::text::vector(1024),p.id LIMIT match_count;
$$;
...
SET search_path = public, extensions, pg_temp AS $$
  ...
  WITH candidates AS MATERIALIZED (
    ...
    WHERE p.embedding IS NOT NULL
      AND (include_expired OR coalesce(p.deadline,p.event_end_date,p.event_start_date) IS NULL OR coalesce(p.deadline,p.event_end_date,p.event_start_date) >= as_of)
